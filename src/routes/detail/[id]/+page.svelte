<script lang="ts">
	...
	import {
		...
		getImageDocuments,
		getSubject
	} from '../../../services/google-functions-service';
	import {
		GeoJSONFeatureCollection,
		ImageDocument,
		Subject
	} from '../../../../sharedModels/interfaces';
		...
		try {
			...
			if (responseGeoJson.ok) {
				...
				showSuccessToast(
					...
					`Now you can explore the map of related images of ${currentSubject.name}`
				);
			} else {
			...
			if (responseGetImages.ok) {
				...
				showSuccessToast(
					...
					"Now you can explore the image archive of Kapellen and it's surroundings."
				);
			} else {
				throwError(responseGeoJson.statusText);
			}
			...
			showErrorToast(
				'Map or Image load failed',
				`It looks like something went wrong! Please contact the admins. ${error}`
			);
		} finally {
			mapLoaded = true;
			imagesLoaded = true;
...
<div class="text-center">
	...
	{:else}
		<h1
			class="m-4 mt-16 text-3xl font-extrabold leading-none tracking-tight text-gray-900 dark:text-gray-100 md:text-4xl lg:text-5xl dark:text-white max-w-5xl mx-auto"
		>
			{currentSubject?.name}
		</h1>
		<p
			class="mb-6 text-lg font-normal text-gray-500 dark:text-gray-400 lg:text-xl sm:px-16 xl:px-48"
		>
...
<br />
<br />
<Map {geoJsonData} {mapLoaded} />
<br />
<br />
{#if imagesLoaded}
	<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
		{#each imageDocuments as imageDocument}
			<ImageCard {imageDocument} />
		{/each}
	</div>
{/if}
