...
/**
 * The kastelen index, prerendered.
 *
 * One of the three places the whole archive can be browsed from, and where the menu's
 * "Alle kastelen" points. The list comes from `load` so it is in the HTML: these pages exist to
...
export async function load({ fetch }) {
	return { places: await placeFamily(fetch, 'kastelen') };
}
