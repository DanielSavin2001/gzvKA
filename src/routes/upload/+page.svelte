<script lang="ts">
    ...
    import {createSubject, retrieveAllSubjects, uploadImages} from "../../services/google-functions-service";
    ...
    onMount(async () => {
        try {
            const response = await retrieveAllSubjects();
            if (response.ok) retrievedSubjects = await response.json();
            else throwError(response.statusText)
        } catch (error) {
            showErrorToast("Upload page load failed", `It looks like something went wrong! Please contact the admins. ${error}`)
        }
    });
    ...
    async function handleSubmitCreationSubject(event: Event) {
        ...
        try {
            ...
            if (responseCreation.ok) retrievedSubjects = await (await retrieveAllSubjects()).json();
