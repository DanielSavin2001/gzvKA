	...
	{:else}
		...
		<ul class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
			{#each stories as story (story.slug)}
				<li>
					<a
						class="flex h-full flex-col rounded-xl border border-gray-200 bg-white p-5 transition hover:border-blue-600 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
						href="/verhaal/{story.slug}"
					>
						<h2 class="text-lg font-bold leading-snug text-gray-900">{story.title}</h2>
		...
		{#if contests.length > 0}
			<section class="mt-12 border-t border-gray-200 pt-8">
				<h2 class="text-xl font-bold text-gray-900">Wedstrijden en zoektochten</h2>
				...
				<ul class="mt-4 flex flex-wrap gap-2">
					{#each contests as story (story.slug)}
						<li>
							<a
								class="inline-block rounded-full border border-gray-300 px-3 py-1.5 text-sm text-gray-800 transition hover:border-blue-700 hover:bg-blue-50"
								href="/verhaal/{story.slug}"
							>
								{story.title}
							</a>
